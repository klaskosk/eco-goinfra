...
var (
	// dummyDeploymentManager is a test deployment manager for use in tests.
	...
	// dummyInventorySubscription is a test inventory subscription for use in tests.
	...
	// dummyOCloudInfo is a test O-Cloud info resource for use in tests.
	...
	// dummyResourceType is a test resource type for use in tests.
	...
	// dummyResourcePool is a test resource pool for use in tests.
	...
	// dummyResource is a test resource for use in tests.
	...
	// dummyLocationInfo is a test location for use in tests.
	...
	// dummyOCloudSiteInfo is a test O-Cloud site for use in tests.
	...
	// dummyInventoryAlarmDictionary is a test alarm dictionary for use in tests.
	...
	// defaultInventorySubscriptionID is the ID from dummyInventorySubscription for use in tests.
	defaultInventorySubscriptionID = *dummyInventorySubscription.SubscriptionId
	// defaultResourceTypeID is the ID from dummyResourceType for use in tests.
	defaultResourceTypeID = dummyResourceType.ResourceTypeId
	// defaultResourcePoolID is the ID from dummyResourcePool for use in tests.
	defaultResourcePoolID = dummyResourcePool.ResourcePoolId
	// defaultResourceID is the ID from dummyResource for use in tests.
	defaultResourceID = dummyResource.ResourceId
	// defaultInventoryAlarmDictID is the ID from dummyInventoryAlarmDictionary for use in tests.
	defaultInventoryAlarmDictID = dummyInventoryAlarmDictionary.AlarmDictionaryId
	// defaultGlobalLocationID is the global location ID from dummyLocationInfo for use in tests.
	defaultGlobalLocationID = dummyLocationInfo.GlobalLocationId
	// defaultOCloudSiteID is the ID from dummyOCloudSiteInfo for use in tests.
...
func TestBuildInventory(t *testing.T) {
	...
	testCases := []struct {
		name          string
		builder       func(serverURL string) *ClientBuilder
		expectedError string
	}{
		{
			name:    "success",
		...
		},
		{
			...
			builder: func(_ string) *ClientBuilder {
				return NewClientBuilder("")
			},
			expectedError: "error: missing base URL",
		},
		{
			name: "nil builder",
			builder: func(_ string) *ClientBuilder {
			...
			expectedError: "error: received nil ClientBuilder",
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			inventoryClient, err := testCase.builder(serverURL).BuildInventory()

			if testCase.expectedError != "" {
				assert.Error(t, err)
				assert.Contains(t, err.Error(), testCase.expectedError)
				assert.Nil(t, inventoryClient)
