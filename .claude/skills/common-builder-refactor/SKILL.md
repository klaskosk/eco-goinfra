...
Refactors a package's `Builder` struct to use `pkg/internal/common`. The `pkg/route` package is the canonical reference.
...
Run `make lint && make test`. Failures are expected — tests still reference the old API. Understand each failure before touching test files.
